...
Targeting is identical to `find`. Exactly one expectation per command:
...
pyselector expect --json --window-handle 0x2E20F46 --auto-id nameBox --value-equals "Yamada"
...
The ones that need a unique target fail with `ambiguous_target` (exit 6) when several elements match; narrow the conditions or pass `--index N`.
...
`origin` is the screen coordinate of the image's top-left corner, so you can line an element `rectangle` up with what you see. Existing files are not overwritten without `--force`.

## `launch` / `close`: put the app in a known state

Both change the machine's state, so they pass the **same two gates as `act`** (`.env` plus `--allow-actions`) and both support `--dry-run`.
...
To get a wait into the generated test, use `expect --wait N`. Nothing is inserted on your behalf.
